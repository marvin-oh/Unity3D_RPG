...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField] private float     minDistance = 5;    // 카메라와 target의 최소 거리
    [SerializeField] private float     maxDistance = 15;   // 카메라와 target의 최대 거리
    [SerializeField] private float     wheelSpeed  = 2000; // 마우스 휠 스크롤 속도
    ...
    private void LateUpdate()
    {
        ...
        // 장비창 카메라 위치 및 회전 조절
        equipCamera.position = target.position + target.rotation * new Vector3(-1, 0, 1.5f);
        equipCamera.LookAt(target);
        equipCamera.position += new Vector3(0, 0.5f, 0);
    }
